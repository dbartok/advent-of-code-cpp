...
std::vector<Particle> createParticlesFromLines(const std::vector<std::string>& particleLines)
{
    ...
    for (auto particleLine : particleLines)
    {
        ...
        Particle::VectorType position{std::stoi(tokens[0]), std::stoi(tokens[1]), std::stoi(tokens[2])};
        Particle::VectorType velocity{std::stoi(tokens[3]), std::stoi(tokens[4]), std::stoi(tokens[5])};
        Particle::VectorType acceleration{std::stoi(tokens[6]), std::stoi(tokens[7]), std::stoi(tokens[8])};

        particles.emplace_back(std::move(position), std::move(velocity), std::move(acceleration));
    }
