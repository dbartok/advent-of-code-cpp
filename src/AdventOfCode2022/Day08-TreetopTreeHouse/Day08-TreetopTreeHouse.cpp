...
private:
    ...
    std::vector<MatrixRow> getSightlinesFrom(int x, int y) const
    {
        ...
        MatrixRow sightlineLeft = m_heightmapMatrix(Eigen::seq(x, x), Eigen::seq(0, y - 1)).reshaped();
        MatrixRow sightlineRight = m_heightmapMatrix(Eigen::seq(x, x), Eigen::seq(y + 1, m_heightmapMatrix.cols() - 1)).reshaped();
        MatrixRow sightlineUp = m_heightmapMatrix(Eigen::seq(0, x - 1), Eigen::seq(y, y)).reshaped();
